...
function copyBtnFunc() {
  ...
  copyBtn.forEach((btn) => {
    ...
    if (btn?.querySelector('span')?.textContent) btn.querySelector('span').textContent = 'COPIED';

    setTimeout(() => {
      btn.classList.remove('copied-clipboard-btn');
      if (btn?.querySelector('span')?.textContent) btn.querySelector('span').textContent = 'COPY LINK'; // Reset the text to original
    }, 2000);
  });
}
...
function getTagsHtml(tags) {
  ...
  const uniquetagsArray = Array.from(new Set(tags.map((tag) => tag.id))).map((id) => {
...
function createAndAppendDropDownOption(container, textContent = 'Select Topic', value = '') {
...
function generateBreadcrumbTrail(currentUrl, name) {
  ...
  let lastBreadcrumbLink = `<a href="#" onclick="location.reload(); return false;">${name ? name : lastBreadcrumb}</a>`;
...
function getProductsByCategory(category_id, category_name = '', fromCategoryDetail = false) {
  // Show the loader
  const container = document.getElementById('productsByCategory');
  const browseCategoriesContainer = document.getElementById('browseCategoriesDiv');
  ...
  fetch(evenFloFAQURL + `products/by/category/${category_id}`)
    ...
    .then((data) => {
      if (data && data?.length > 0) {
        ...
        const blocksToShow = productsBlock ? productsBlock?.dataset?.faqsToShow : 3;
        ...
        if (browseCategoriesContainer) {
          ...
          browseCategories.innerHTML = `<button class="button">Browse All ${category_name}</button>`;
